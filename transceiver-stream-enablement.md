...
struct SframeReceiveConfig {
  bool required = false;
  scoped_refptr<SframeMediaDecryptorInterface> decryptor;
};
```

The configuration is consumed **by value** down the media/send pipeline; the
`scoped_refptr` keeps the encryptor/decryptor alive for as long as the sender or
...
| Sender/receiver implementation | `RtpSenderBase`, `RtpReceiverBase` | Notify the owning transceiver that SFrame is desired, create the media encryptor/decryptor, and attach it to the channel once the SSRC is known. |
...
| RTP send path | `RTPSenderVideo` | Drop frames while SFrame is required but no encryptor is attached; encrypt frames or packets once the encryptor is available. |
| RTP receive path | `RtpVideoStreamReceiver2` | Route SFrame packets through descriptor parsing, buffering, and decryption; drop while the decryptor is missing. |
...
| Step | What happens |
|---|---|
| 1. Application opts in | The application asks a sender or receiver for an SFrame key-management handle (`CreateSframeEncryptorOrError` / `CreateSframeDecryptorOrError`). |
...
| 6. Media crypto is attached | When the encryptor/decryptor is available for an SSRC, `SetSframeEncryptor`/`SetSframeDecryptor` fills in the config's crypto field and recreates the stream. |
...
sequenceDiagram
    ...
    App->>Tx: AddTransceiver(video, sendrecv)
    Tx-->>App: transceiver (sframe_enabled_ = nullopt)
    App->>Sender: CreateSframeEncryptorOrError()
    Sender->>Tx: enable_sframe_at_owner_()
    Tx-->>Sender: sframe_enabled_ = true (renegotiation needed)
    Sender-->>App: OK (encryptor pending SSRC)
    App->>Recv: CreateSframeDecryptorOrError()
    Recv-->>App: OK (decryptor pending SSRC)

    Note over App,SOA: CreateOffer emits a=sframe
    App->>SOA: CreateOffer
    SOA->>SOA: options.sframe_enabled = Tx.SframeEnabled() = true
    ...
    VSC-->>Ch: SframeSendConfig.required = true (no encryptor)
    Sender->>VSC: SetSsrcTask -> SetSframeEncryptor(ssrc)
    VSC-->>Sender: SframeSendConfig.encryptor set + stream recreated
    ...
    Recv->>VRC: SetSframeDecryptor(ssrc)
    VRC-->>Recv: SframeReceiveConfig.decryptor set + stream recreated
    SOA-->>App: stable
```

### Flow 2 — Passive answerer (recvonly)

The answerer adds no local track and does not create an encryptor/decryptor.
...
sequenceDiagram
    ...
    App->>SOA: CreateAnswer
    SOA->>SOA: NegotiateSframeUsage(offer=true, option=true) = true
    SOA-->>App: answer SDP (contains a=sframe)
...
**directly** via `SetSframeDecryptor` (the receive SSRC is already known from the
...
sequenceDiagram
    ...
    App->>Sender: CreateSframeEncryptorOrError()
    Sender-->>App: OK (encryptor stored, ssrc==0 -> deferred)
    App->>Recv: CreateSframeDecryptorOrError()
    Recv->>VRC: SetSframeDecryptor(ssrc) (signaled SSRC known)
    VRC-->>Recv: SframeReceiveConfig.required = true + decryptor set + recreated
    ...
    VSC-->>Ch: SframeSendConfig.required = true (no encryptor)
    Sender->>VSC: SetSsrcTask -> SetSframeEncryptor(ssrc)
    VSC-->>Sender: SframeSendConfig.encryptor set + recreated
    SOA-->>App: stable
```

Because the stream-level `EnableSframe()` is idempotent, the answer-time
send-side fan-out never clobbers the decryptor installed directly during enable.
...
encryptor. If `required` is set but `encryptor` is still null, `RTPSenderVideo`
drops the frame before packet allocation or packetization.

When the encryptor is attached, the send path applies the sender-selected SFrame
mode:
...
media decryptor. If `required` is set but `decryptor` is still null, incoming
SFrame media is dropped instead of being forwarded to the decoder pipeline.

When the decryptor is attached, the receive path derives the processing mode
...
- **Receive crypto is order-independent.** A receive SSRC is already known from
  the remote offer, so the decryptor may be installed before the latch flips.
  `SetSframeDecryptor` sets `required = true` and stores the decryptor
  unconditionally; a later fan-out is then a no-op. Either ordering converges on
  `required == true` with the decryptor attached.
- **Send never races.** The sender's SSRC is `0` until negotiation, so its
  encryptor push is deferred to `RtpSenderBase::SetSsrcTask`, which runs after
...
| Step | Function | File |
|---|---|---|
| Sender creates encryptor, latches transceiver | `RtpSenderBase::CreateSframeEncryptorOrError` | `pc/rtp_sender.cc` |
| Receiver creates decryptor, latches transceiver | `RtpReceiverBase::CreateSframeDecryptorOrError` | `pc/rtp_receiver.cc` |
...
| Send stream config (required + encryptor) | `WebRtcVideoSendStream::EnableSframe` / `SetSframeEncryptor` | `media/engine/webrtc_video_engine.cc` |
| Receive stream config (required + decryptor) | `WebRtcVideoReceiveStream::EnableSframe` / `SetSframeDecryptor` | `media/engine/webrtc_video_engine.cc` |
